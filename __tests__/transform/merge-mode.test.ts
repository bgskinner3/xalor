...
describe('Runtime Generator API', () => {
  ...
  describe('🧩 ADVANCED MASKING & VALUE PROJECTIONS (V0 OPTION MATRIX)', () => {
    it('🎯 TRACK 4: should merge object graphs and apply pick, omit, and map parameters simultaneously inside a single pass', () => {
      ...
      // Executes your unified, 3-line composite public engine signature
      const result = xalor.merge<'USER_TEST'>({
        dataOne: baselineState,
        dataTwo: patchDelta,
        // Retains only these properties at the root layer
        pick: ['id', 'username', 'active'],
        // Strips out explicit security parameters flatly
        omit: ['active'],
        // 🚀 Zod-style mapping value projection running single-pass!
        map: {
      ...
      // Verify data values mapped inline successfully
      expect(result).toMatchObject({
        id: 1505,
        username: 'SKINNERMORPHED',
      });

      // // Verify masking parameters removed explicit fields cleanly
      expect(Reflect.has(result, 'active')).toBe(false);
    });
  });
  //============================================================================================
  //============================================================================================
