...
export const TRANSFORM_SHAPE_MAPPER: TUniversalTransformMapper = {
  ...
  literal: (shape, data, dependency) => {
    if (dependency.mode === 'merge') {
      const activeValue = !isUndefined(dependency.patchData)
  ...
  intersection: (shape, data, dependency, depth, recurse) => {
    ...
    for (const branch of shape.values) {
      ...
      if (isNull(acc) || isUndefined(acc)) return null;
    }
...
export const TRANSFORM_FLATTEN_MAPPER: TShapeFlattenMapper = {
  ...
  instanceof: (shape, val, acc, path) => {
    const c = shapeKindUtilsService.resolveInstanceCtor(shape.name);
    if (!(val instanceof c)) return;

    const serialized = shapeKindUtilsService.getInstanceOfKind(shape.name);
    if (!isUndefined(serialized)) {
      acc[path] = serialized.def();
    }
  },
